...
public class UnitySynthTest : MonoBehaviour
{
	...
	public string midiFilePath = "Midis/M4.mid";
	//Try also: "FM Bank/fm" or "Analog Bank/analog" for some different sounds
	public string bankFilePath = "GM Bank/gm";
	public int bufferSize = 1024;
	public int midiNote = 20;
	public int midiNoteVolume = 0;
	...
	public Transform nota_normal, nota_riscada;
	private int notaa;
	private bool pode_inst = false;
	...
	void Start ()
	{
	//	InvokeRepeating ("Inst", 0, 0.18475f);
	}
   ...
   /*     if (Input.GetKeyDown(KeyCode.A))
		...
		*/
		...
		if (midiSequencer.isPlaying && pode_inst)
		{
				if (notaa == 60)
					Instantiate (nota_riscada, new Vector3 (midiSequencer.SampleTime/7156.1f, notaa/2, 0), Quaternion.identity);
				else if (notaa % 2 == 0)
					Instantiate (nota_normal, new Vector3 (midiSequencer.SampleTime/7156.1f, (notaa/2), 0), Quaternion.identity);
				else 
					Instantiate (nota_normal, new Vector3 (midiSequencer.SampleTime/7156.1f, (notaa/2) + 1, 0), Quaternion.identity);
	...
	void Inst(){
		...
		{
		//	if (note%2 == 0)
		//		Instantiate (nota1, new Vector3 (midiSequencer.SampleTime/7156.1f, notaa/2, 0), Quaternion.identity);
		//	pode_inst = false;
		//	else 
		//		Instantiate (nota1, new Vector3 (midiSequencer.SampleTime/8500, (notaa/2), 0), Quaternion.identity);
		}	
	}
	...
	void OnGUI ()
	{
		...
		if (GUILayout.Button ("Play Song")) {
			midiStreamSynthesizer.MasterVolume = 0f;
			midiSequencer.Play ();
		}
	...
	public void MidiNoteOnHandler (int channel, int note, int velocity)
	{
		notaa = note;
		pode_inst = true;
		Debug.Log ("NoteOn: " + note.ToString () + " Velocity: " + velocity.ToString () + " Time: " + midiSequencer.SampleTime);
//		pode_inst = false;

	//	Instantiate(nota1, new Vector3(1, note, 0), Quaternion.identity);
	}
